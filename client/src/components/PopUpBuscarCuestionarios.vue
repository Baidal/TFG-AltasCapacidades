<template>
    <div class="fixed top-0 left-0 w-screen h-screen bg-gray-100 opacity-90 z-20">
        <div class="absolute top-1/3 left-1/2 -mt-12 -ml-80 w-1/3 border-2 border-black rounded-md shadow-md opacity-100 p-2 bg-gray-50 flex flex-col items-center">
            ...
            <div class="mt-4 w-full grid grid-cols-4">
                <TarjetaCuestionario v-for="cuestionario in cuestionarios" 
                    :key="cuestionario.id" 
                    :nombre="cuestionario.nombre" 
                    :id="cuestionario.id"
                    :showRemoveButton="false"
                    :class="['mx-1', 'my-1', this.cuestionarioSeleccionado(cuestionario.id) ? 'bg-green-200' : '', 'cursor-pointer']"
                    @click="anyadirCuestionarioASeleccionados(cuestionario)"
                    />
            </div>

            <AppButton :name="'Añadir cuestionarios'" class="mt-4 w-42" @click="$emit('anyadirCuestionarios', this.cuestionariosSeleccionados)"/>
        </div>
    </div>
</template>
